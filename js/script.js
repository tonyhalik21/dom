...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function initializeMainApp() {
        ...
        const sendBrightness = debounce((value) => {
            sendSocketMessage({ action: "set_brightness", value: value });
        }, 150);
    
        brightnessSlider.addEventListener('input', () => {
            sendBrightness(parseInt(brightnessSlider.value, 10));
        });
    }
});
...
function updateUI(data) {
    ...
    updateLedControl(data.ledState, data.ledSource, data.timeRemaining, data.maxBrightness);
...
function toggleLed() {
    ...
    ledCard.classList.add('loading');
    
    sendSocketMessage({ action: "toggle_www" });
}

function updateLedControl(state, source, timeRemaining, maxBrightness) {
    const statusText = document.getElementById("ledStatus");
    ...
    const ledCard = document.getElementById("ledControlCard");
    const brightnessContainer = document.getElementById('brightnessControlContainer');
    const brightnessSlider = document.getElementById('brightnessSlider');
    const toggleButton = document.querySelector('.toggle-button');

    ledCard.classList.remove('loading');
    ...
    if (state) {
        ...
        if (document.activeElement !== brightnessSlider) {
            brightnessSlider.value = maxBrightness;
        }
        ...
        if (timeRemaining > 0) {
            ...
            function renderCountdown() {
                timerText.innerText = `Wyłączy się za ${Math.floor(remaining / 60)}m ${String(remaining % 60).padStart(2, "0")}s`;
                progressBar.style.width = `${Math.min(100, (remaining / totalDuration) * 100)}%`;
...
function createLogElement(log) {
    const logIcons = {
        "PIR": "fa-person-rays",
        "WWW": "fa-globe",
        "Przycisk": "fa-hand-pointer",
        "BTN": "fa-hand-pointer",
        "LDR": "fa-gauge-high",
        "TRYB": "fa-gears",
        "Wyłączony": "fa-power-off",
        "Błąd": "fa-triangle-exclamation",
